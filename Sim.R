...
initial_players <-matrix(c(60*3+10,12,1,
                           60*3+11,11,1),ncol=3,byrow=T)
...
store_frames <- matrix(nrow=steps,ncol=columns*rows)
  ...
  person_group_1[moves_person_1_seed,3]<-ceiling(runif(min=0,max=2,n=count_person_1)+0.0000001)[moves_person_1_seed]
  person_group_1 <- rbind(c(NA,NA,NA),person_group_1)
  players <- rbind(person_group_1, subset(players,players[,"Type"]!=11) )
  ...
  does_die <- c(FALSE,does_die) #Keeps the NA row always
  person_group_2 <- person_group_1[!does_die,] #Keep the humans 1 that don't die
  ...
  person_group_2[moves_person_2_seed,3]<-ceiling(runif(min=0,max=2,n=count_person_1)+0.0000001)[moves_person_2_seed]
  person_group_2 <- rbind(c(NA,NA,NA),person_group_2)
  players <- rbind(person_group_2, subset(players,players[,"Type"]!=12) )
...
for (frame_index in 160:240) {
  all_coords <- NULL
  for (rownum in 1:rows){
    coords <- cbind(rep(rownum,columns),1:columns,0)
  ...
  gfx_obj <- ggplot(data=as.data.frame(all_coords),aes(x=X_component,y=Y_component,col=as.factor(Color_component)))+
    scale_color_manual(values = c("green","blue","blue","yellow","orange"))+
  ...
  columns <- 1000
  rows    <- 800
all_coords <- cbind(rep(c(1:rows),each=columns),rep(c(1:columns),times=rows))
